...
```{r}
floyd <- function(no_origem, no_destino, Cij, labels=NULL, print=TRUE) {
  ...
  ### Parte 2: Recuperação do trajeto
...
ggnet2(net_sub, label = T)

trajeto <- floyd(no_origem=2, no_destino=8, Cij=Cij_sub, labels=nos_sub)[[1]]
...
Cij_sub2_floyd <- floyd(no_origem=4, no_destino=19, Cij=Cij_sub2, labels=nos_sub2, print=FALSE)[[2]]

set.seed(10)
net_sub2_floyd <- network(Cij_sub2_floyd, directed = F)
ggnet2(net_sub2_floyd, label = T)
```


```{r}
# Algoritmo de savings considerando o nó 12 como depósito
